...
### 公開HTML確認時のsandbox DNS制限

- 確認日: 2026-09-07
- コマンド: `curl -fsSL --max-time 20 'https://sora-gallery.pages.dev/video/公開ID'`
- エラー概要: 通常実行では `Could not resolve host` になった。
- 次回避けること: 同じ通常実行を繰り返して公開障害と判断しない。
- 推奨手順・確認結果: ネットワーク許可付きの読み取りで再確認し、HTML取得に成功した。
