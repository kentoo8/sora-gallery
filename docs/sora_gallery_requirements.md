...
- PC 版とスマホ版の縦ツール列は、いいね、ミュート、自動再生、シャッフルの順を揃える。
- PC 版の prompt 横にはコピー操作だけを残し、いいねは縦ツール列へ置く。
...
- `R`: シャッフルモードのオン/オフ。
- `?`: ショートカット表示。
- デスクトップ版では、再生画面上のトラックパッド二本指上下フリックでも前後動画に移動する。
- デスクトップ版の横フリックはアプリ側で捕捉せず、ブラウザ標準に譲る。
...
- 横スワイプ/横フリックによる履歴移動はアプリ側で捕捉せず、ブラウザ標準に譲る。
- ブラウザ標準の戻る操作や OS ジェスチャーとの衝突を避ける。
- 再生画面のタグから一覧へ戻る場合やタグ絞り込みを切り替える場合は、一覧の縦スクロール位置を動画一覧の頭へ移動する。
- 再生画面のタグ一覧は縦幅を増やさず、横スクロールで続きのタグを見られるようにする。
