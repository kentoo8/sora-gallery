...
- シャッフルモードはページロードごとに全動画のランダム順を一度だけ作り、そのセッション中は固定順として扱う。
- シャッフルモードがオンの間は、次へ進む操作、戻る操作、動画終了時の自動再生による移動先を、そのセッション固定のシャッフル順で決める。
- 検索/タグ絞り込み中のシャッフル移動は、セッション固定のシャッフル順から該当動画だけを抜き出した順で進む/戻る。
- PC 版とスマホ版の縦ツール列は、いいね、ミュート、自動再生、シャッフルの順を揃える。
- PC 版の prompt 横にはコピー操作だけを残し、いいねは縦ツール列へ置く。
- 個別動画 URL を初期リリースから提供する。
- 個別動画 URL の初期HTMLに共有用メタ情報を含め、対応サービスでサムネイルと説明文を表示できるようにする。ビルド時に公開データから生成し、共有先での動画再生は保証しない。
