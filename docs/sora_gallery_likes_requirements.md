...
*   **処理ロジック**:
    ...
    3.  同一IPアドレスからの短時間での連打を防ぐための「簡易レートリミット」（後述）を適用。
    4.  SQLの `INSERT ... ON CONFLICT(video_id) DO UPDATE` (Upsert句) を実行し、カウントを `+1` する。
        ```sql
...
*   同一IPからの短時間での連続リクエストがあった場合、ステータスコード `429 Too Many Requests` を返却する。
...
### 5.3 将来の強化候補
likes をランキング、集計表示、公開指標としてより重く扱う場合は、以下のいずれかを検討する。

*   Cloudflare Turnstile による bot 対策。
*   D1 に匿名クライアント識別子または短期ハッシュを保存し、動画ごとの重複送信を抑制する。
*   Cloudflare Rate Limiting / WAF など、エッジ側の製品機能で API 単位の制限を行う。
...
*   preview で likes API を有効にする場合は、別途 `sora-gallery-likes-db-preview` のような preview 専用 D1 database を作る。
*   preview 用 D1 を作成したら、`wrangler.toml` の `[[env.preview.d1_databases]]` に `binding = "DB"` と preview database の `database_id` を追加する。
*   preview 用 D1 にも `schema.sql` を適用する。
*   preview 用 D1 が未作成の間は、preview 環境で本番 likes DB を操作しない。
...
database_id = "<preview-d1-database-id>"
```
